...
public class UserControllerTest {
    ...
    static Users user = new Users("asdf", "", "asdf", 26, "", false, new ArrayList<>(), Role.USER);
...
//                .andExpect(jsonPath("$.userNickname", is("fdsa")))
                .andDo(print());
    }

    @Test
    public void update() throws Exception {
        mvc.perform(post("/api/v1/user/update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(user)))
                .andExpect(status().isOk())
...
//                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void getUser() throws Exception {
        mvc.perform(post("/api/v1/user/getuser")
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(user)))
                // status 검증. 잘 수행되면 200
                .andExpect(status().isOk())
